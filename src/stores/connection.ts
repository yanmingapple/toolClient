 ...
 */
const savePersistedState = async (connections: TreeNode[]) => {
  try {
    // 从TreeNode中提取ConnectionConfig数据
    const connectionConfigs: ConnectionConfig[] = connections
      .map(node => node.connectionConfig)
      .filter(config => config !== undefined) as ConnectionConfig[]
    await saveDatabaseConnections(connectionConfigs)
  } catch (error) {
