      ...
      <div class="header-content">
        <div class="header-left">
          <div class="logo-section">
            <div class="logo-icon paddleocr">
              <el-icon><Document /></el-icon>
            </div>
            <h1 class="ocr-title">PaddleOCR 文字识别</h1>
          </div>
    ...
    <main class="ocr-main">
      ...
      <section class="panel options-section">
        ...
        <div class="options-container">
          ...
          <!-- 自动旋转 -->
          <div class="option-item">
            <label class="option-label">自动旋转</label>
            <el-switch v-model="autoRotate" />
          </div>

          <!-- 灰度化 -->
          <div class="option-item">
            <label class="option-label">灰度化</label>
            <el-switch v-model="grayScale" />
          </div>

          <!-- 对比度 -->
          <div class="option-item">
            <label class="option-label">对比度: {{ contrast.toFixed(1) }}</label>
            <el-slider v-model="contrast" :min="0.5" :max="2.0" :step="0.1" />
          </div>

          <!-- 亮度 -->
          <div class="option-item">
            <label class="option-label">亮度: {{ brightness }}</label>
            <el-slider v-model="brightness" :min="-50" :max="50" :step="1" />
          </div>
        </div>
      </section>
...
import { Upload, Document, Delete, MagicStick, CopyDocument, Download, Loading, Setting } from '@element-plus/icons-vue'
import { ElMessage } from 'element-plus'
import { paddleOCR } from './engines/paddleocr/PaddleOCR'
...
const startRecognition = async () => {
  ...
  progress.value = 0
  progressMessage.value = '正在初始化 PaddleOCR 引擎...'
...
debugger
  try {
    ...
    progress.value = 20
    progressMessage.value = 'PaddleOCR 引擎初始化完成'
    ...
    for (let i = 0; i < selectedFiles.value.length; i++) {
      ...
      progress.value = 20 + Math.round(((i + 1) / selectedFiles.value.length) * 60)

      // 将图片转换为 ImageData
      const imageData = await loadImageData(file.preview)

      // 识别图片
      const result = await paddleOCR.recognize(imageData)
...
const exportResult = () => {
  ...
  a.href = url
  a.download = 'paddleocr_result_' + Date.now() + '.txt'
...
onMounted(() => {
  console.log('PaddleOCR page mounted')
})
