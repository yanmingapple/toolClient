<template>
  <div class="ocr-container" @paste="handlePaste">
      ...
      <div class="header-content">
        <div class="header-left">
          <div class="logo-section">
            <div class="logo-icon tesseract">
              <el-icon><MagicStick /></el-icon>
            </div>
            <h1 class="ocr-title">Tesseract OCR 文字识别</h1>
          </div>
    ...
    <main class="ocr-main">
      ...
      <!-- 右侧：识别结果 -->
      <section class="panel result-section full-width">
        <div class="panel-header">
          <div class="panel-icon">
            <el-icon><Document /></el-icon>
          </div>
...
const startRecognition = async () => {
  ...
  progress.value = 0
  progressMessage.value = '正在初始化 Tesseract 引擎...'
  recognizedText.value = ''

  try {
    // 创建Tesseract引擎实例
    const engine = await ocrEngineFactory.createEngine('tesseract')
...
const exportResult = () => {
  ...
  a.href = url
  a.download = 'tesseract_result_' + Date.now() + '.txt'
...
onMounted(() => {
  console.log('Tesseract page mounted')
})
...
@keyframes pulse-glow {
  ...
  50% {
    box-shadow: 0 4px 20px rgba(102, 126, 234, 0.5), 0 0 30px rgba(102, 126, 234, 0.2);
  }
}
