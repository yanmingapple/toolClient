      ...
      <div class="header-content">
        <div class="header-left">
          <div class="logo-section">
            <div class="logo-icon deepseek">
              <el-icon><ChatDotRound /></el-icon>
            </div>
            <h1 class="ocr-title">DeepSeek 多模态识别</h1>
          </div>
      ...
      <section class="panel options-section">
        ...
        <div class="options-container">
          ...
          <div class="option-item">
            ...
            <el-select v-model="detailLevel" placeholder="选择详细程度" size="large">
...
import { Upload, Document, Delete, MagicStick, CopyDocument, Download, Loading, Setting, ChatDotRound } from '@element-plus/icons-vue'
...
const prompt = ref('')
const detailLevel = ref<string>('standard')
...
const startRecognition = async () => {
  ...
  progress.value = 0
  progressMessage.value = '正在连接 DeepSeek 服务...'
  recognizedText.value = ''

  try {
    // 创建DeepSeek引擎实例
    const engine = await ocrEngineFactory.createEngine('deepseek')
...
const exportResult = () => {
  ...
  a.href = url
  a.download = 'deepseek_result_' + Date.now() + '.txt'
...
onMounted(() => {
  console.log('DeepSeek page mounted')
})
...
@keyframes pulse-glow {
  ...
  50% {
    box-shadow: 0 4px 20px rgba(59, 130, 246, 0.5), 0 0 30px rgba(59, 130, 246, 0.2);
  }
}
