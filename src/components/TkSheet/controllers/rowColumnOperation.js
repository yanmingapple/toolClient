...
import { isRealNull, isRealNum, hasPartMC, isCellEditMode, isEditMode, checkIsAllowEdit } from "../global/validate";
...
export function rowColumnOperationInitial() {
    ...
    $("#luckysheet-rows-h")
        ...
        .mouseup(function(event) {
            if (event.which == 3) {
                //2024-5-7添加，兼容单元格设置了不可编辑属性，屏蔽右键，该地方是最左边的右键触发，故只需要找到这个行数据是否有设置不可编辑属性
                let mouse = mouseposition(event.pageX, event.pageY);
                let y = mouse[1] + $("#luckysheet-rows-h").scrollTop();
                ...
                row_index = row_location[2],
                cellIsEditor = true;
                for(var a = 0;a < Store.flowdata[row_index].length;a++){
                    const b = Store.flowdata[row_index][a]
                    if(b && b.allowEdit == false){
                        cellIsEditor = false;
                        break;
                    }
                    else{
                ...
                cellRightClickConfig.hideRow ? $("#luckysheet-show-selected").show() : $("#luckysheet-show-selected").hide();
                cellRightClickConfig.rowHeight
                ...
                if (!cellRightClickConfig.copy && !cellRightClickConfig.copyAs && !cellRightClickConfig.paste) {
                    ...
                    if (
                        ...
                        $("#luckysheet-cols-rows-data .luckysheet-menuseparator").hide(0);
                    }
                }
        ...
        .mouseup(function(event) {
            if (event.which == 3) {
               ...
               cellRightClickConfig.insertColumn
                ...
                cellRightClickConfig.deleteColumn 
                ? $("#luckysheet-del-selected").show()
                :  $("#luckysheet-del-selected").hide();

                cellRightClickConfig.hideColumn 
                ? $("#luckysheet-hide-selected").show()
                :  $("#luckysheet-hide-selected").hide();
               cellRightClickConfig.hideColumn  
                ? $("#luckysheet-show-selected").show()
                :  $("#luckysheet-show-selected").hide();
                cellRightClickConfig.columnWidth
                ? $("#luckysheet-column-row-width-selected").show()
                :  $("#luckysheet-column-row-width-selected").hide();
    ...
    $("#luckysheet-cols-menu-btn").click(function(event) {
        ...
        $("#luckysheet-cols-rows-add .luckysheet-menuseparator").show();
        $("#luckysheet-cols-rows-shift .luckysheet-menuseparator").show();
    ...
    cellRightClickConfig.hideColumn  
        ...
        if (!cellRightClickConfig.copy && !cellRightClickConfig.copyAs && !cellRightClickConfig.paste) {
            ...
            if (
                ...
                $("#luckysheet-cols-rows-shift .luckysheet-menuseparator").hide();
            }
        }
