...
const luckysheetPostil = {
            ...
            if(data[r]){
                for(let c = 0; c < data[r].length; c++){
                    if(data[r][c] != null && data[r][c].ps != null){
                        let postil = data[r][c]? data[r][c].ps:null;
                        if(postil) _this.buildPs(r, c, postil);
                    }
                }
            }
        }
    ...
    newPs: function(r, c,comment){
        if(!checkProtectionAuthorityNormal(Store.currentSheetIndex, "editObjects")){
            return;
        }

        // Hook function
        const currentCell = (Store.flowdata[r] && Store.flowdata[r][c]) ? Store.flowdata[r][c] : null;
        if(!method.createHookFunction('commentInsertBefore',r,c, currentCell)){
            return;
        }
        ...
        let html =  '<div id="luckysheet-postil-show_'+ r +'_'+ c +'" class="luckysheet-postil-show luckysheet-postil-show-active">' +
                        ...
                        '<div class="luckysheet-postil-show-main" style="width:'+ width +'px;height:'+ height +'px;color:#000;padding:5px;border:1px solid #000;background-color:rgb(255,255,225);position:absolute;left:'+ fromX +'px;top:'+ fromY +'px;box-sizing:border-box;z-index:100;">' +
                            ...
                            '</div>' +
                            '<div style="width:100%;height:100%;overflow:hidden;">' +
                                '<div class="formulaInputFocus" style="width:132px;height:72px;line-height:20px;box-sizing:border-box;text-align: left;word-break:break-all;" spellcheck="false" contenteditable="true">' + commentDivs +
                                '</div>' +
                            '</div>' +
                        '</div>' +
                    '</div>';
        ...
        d[r][c].ps = { "left": null, "top": null, "width": null, "height": null, "value": comment, "isshow": false };
        rc.push(r + "_" + c);
        ...
        if($("#luckysheet-postil-show_"+ r +"_"+ c).length > 0){
            ...
            let html =  '<div id="luckysheet-postil-show_'+ r +'_'+ c +'" class="luckysheet-postil-show luckysheet-postil-show-active">' +
                            ...
                            '<div class="luckysheet-postil-show-main" style="width:'+ width +'px;height:'+ height +'px;color:#000;padding:5px;border:1px solid #000;background-color:rgb(255,255,225);position:absolute;left:'+ left +'px;top:'+ top +'px;box-sizing:border-box;z-index:100;">' +
                                ...
                                '</div>' +
                                '<div style="width:100%;height:100%;overflow:hidden;">' +
                                    '<div class="formulaInputFocus" style="width:'+ (width - 12) +'px;height:'+ (height - 12) +'px;line-height:20px;box-sizing:border-box;text-align: left;;word-break:break-all;" spellcheck="false" contenteditable="true">' +
                                        commentDivs +
                                    '</div>' +
                                '</div>' +
                            '</div>' +
    ...
    delPs: function(r, c){
        ...
        // 安全检查：确保单元格存在
        if(!Store.flowdata[r] || !Store.flowdata[r][c]){
            return;
        }

        const previousCell = Store.flowdata[r][c] ? $.extend(true, {}, Store.flowdata[r][c]) : null;

        // Hook function
        if(!method.createHookFunction('commentDeleteBefore',r,c, previousCell)){
            return;
        }
        ...
        // Hook function
        setTimeout(() => {
            // 使用删除前的单元格数据，因为删除后可能不存在
            method.createHookFunction('commentDeleteAfter',r,c, previousCell)
        }, 0);
    },
    ...
    removeActivePs: function(){
        ...
        if($("#luckysheet-postil-showBoxs .luckysheet-postil-show-active").length > 0){
            ...
            if(!method.createHookFunction('commentUpdateBefore',r,c,value)){
                if (Store.flowdata[r] && Store.flowdata[r][c] && Store.flowdata[r][c].ps && !Store.flowdata[r][c].ps.isshow) {
                    $("#" + id).remove();
                }
                return;
            }

            const previousCell = Store.flowdata[r] && Store.flowdata[r][c] ? $.extend(true,{},Store.flowdata[r][c]) : null;
    ...
    hideResize:function(r,c){
        if($("#luckysheet-postil-show_"+ r +"_"+ c).length > 0){
            $("#luckysheet-postil-show_"+ r +"_"+ c).find(".luckysheet-postil-dialog-resize").hide();
        }
    },
