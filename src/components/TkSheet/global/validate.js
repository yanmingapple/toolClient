...
//单元格是否为非编辑模式
function isCellEditMode() {
    let obj_s = Store.luckysheet_select_save[0];
    let rowIndex = obj_s["row"][0]
    let columnIndex = obj_s["column"][0];

    if(Store.flowdata[rowIndex] && Store.flowdata[rowIndex][columnIndex] && Store.flowdata[rowIndex][columnIndex].allowEdit == false){
        return false;
    }
    else{
        return true;
