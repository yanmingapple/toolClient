<template>
  <div class="height100">
    <iframe id="paperAnswerEditorRender" class="height100 width100" :src="'static/tk_preview_ui/paperAnswerViewEditor.html?project='+project"></iframe>
  </div>
</template>
...
const props = defineProps({
  ...
  , optList = [
    { label: '预览答案排版', type: 'success', optType: 'preview'},
    { label: '重置答案排版', type: 'warning', optType: 'reset'},
    { label: '保存答案排版', type: 'primary', optType: 'save', funcName: 'saveHandler'},
...
// 设置答案数据
function postPaperData(paperAnswerData) {
  var wn = document.getElementById('paperAnswerEditorRender').contentWindow;
  wn.onload = function() {
    wn.postMessage({paperAnswerData, optList}, '*');
  }
}
// 初始化组件，渲染数据
function doComInit (paperAnswerData) {
  paperId = paperAnswerData?.paper?.[0]?.paperId
  setBtnsVisible()
  postPaperData(paperAnswerData)
}
