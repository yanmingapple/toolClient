...
const ViewPanel = ({
  ...
  const nameColumn = {
    ...
    key: 'name',
    width: 200,
    render: (text: string, record: ViewData) => (
      <span
  ...
  return (
    <div style={{ height: '100%', display: 'flex', flexDirection: 'column' }}>
                ...
                try {
                    ...
                    <div style={{ padding: 16, height: '100%', overflow: 'auto' }}>
                      <Spin spinning={tableLoading}>
                        ...
                        <pre style={{ backgroundColor: '#f5f5f5', padding: '10px', borderRadius: '4px', whiteSpace: 'pre-wrap' }}>
                          {objectToOpen.definition}
                        </pre>
                      </Spin>
                    </div>
                  )
                  mainPanelRef.current.createPanel('view', objectToOpen.name, viewContent)
                } catch (error) {
                  ...
                  const errorContent = (
                    <div style={{ padding: 16, height: '100%', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                      <div>
                        <h3>获取数据失败</h3>
                        <p>无法获取视图数据，请检查数据库连接和权限。</p>
                      </div>
                    </div>
                  )
                  mainPanelRef.current.createPanel('view', objectToOpen.name, errorContent)
                } finally {
      ...
      <Table
        ...
        locale={{ emptyText: <Empty description="当前没有视图数据" /> }}
        onRow={(record) => ({
          onClick: () => {
            onSelectObjects?.([record])
            onOpenObject?.(record)
