  ...
  return (
    <div style={{ height: '100%', display: 'flex', flexDirection: 'column' }}>
            ...
            onClick={async () => {
              ...
              if (objectToOpen && mainPanelRef?.current && connectionId && databaseName) {
                ...
                try {
                  ...
                  mainPanelRef.current.createPanel('function', objectToOpen.name, functionContent)
                } catch (error) {
                  ...
                  const errorContent = (
                    <div style={{ padding: 16, height: '100%', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                      <div>
                        <h3>获取数据失败</h3>
                        <p>无法获取函数数据，请检查数据库连接和权限。</p>
                      </div>
                    </div>
                  )
                  mainPanelRef.current.createPanel('function', objectToOpen.name, errorContent)
                } finally {
      ...
      <Table
        ...
        locale={{ emptyText: <Empty description="当前没有函数数据" /> }}
        onRow={(record) => ({
          onClick: () => {
            onSelectObjects?.([record])
            onOpenObject?.(record)
