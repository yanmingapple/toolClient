    ...
    <div class="cus-form">
      ...
      >
        <template v-for="(section, index) in sections" :key="index">
          <TKCustomCollapse :title="section.label" :svgIcon="section.svgIcon" class="mb10">
            <div
              :class="[
              ...
              v-for="(item, index) in getSectionFormConfig(section).filter((el) => {
            ...
            >
              ...
              <template v-else>
                <el-form-item
                  ...
                  :label="(item.label ? item.label : '') + (!item.label || searchFormObj.hideColon ? '' : '：')"
                  :prop="item.prop"
                  :required="item.isRequired"
                  v-if="
                    !(item.hideItem || (item.hiddenFun && typeof item.hiddenFun === 'function' && item.hiddenFun(item)))
                  "
                >
                  <!-- 输入框 -->
                  <el-input
                  ...
                  <el-input
                    ...
                    :placeholder="
                      item.placeholder ||
                      `请输入${item.label || '备注'}   最多${item.maxlength || 200}个字符(包括英文，汉字)`
                    "
                    :rows="item.rows"
                    :show-word-limit="searchFormObj.showWordLimit && !item.hiddenWordLimit"
                    @change="item.change && item.change(searchFormObj.formData[item.prop])"
                  ...
                  <div class="input-range-con" v-if="item.type === 'inputRange'" :style="{ width: item.width }">
                    <el-input-number
                      class="editNum"
                      :maxlength="3"
                  ...
                  <el-radio-group
                    ...
                    @change="item.change && item.change(searchFormObj.formData[item.prop])"
                  >
                    <el-radio v-for="radio in item.radios" :value="radio.value" :key="radio.label">{{
                      radio.label
                    }}</el-radio>
                  </el-radio-group>
                  ...
                  <ul v-if="item.type === 'uploadImgage'" class="el-upload-list el-upload-list--picture-card">
                    <li
                      tabindex="0"
                      ...
                      :key="index"
                    >
                      <img :src="subItem.url" alt class="el-upload-list__item-thumbnail" />
                      ...
                      <span class="el-upload-list__item-actions">
                        <span class="el-upload-list__item-preview" @click="handlePictureCardPreview(subItem)">
                  ...
                  <el-upload
                    ...
                    action="#"
                    :accept="item.accept"
                    :http-request="(data)=>{item.action(data)}"
                    :data="item.data || { action: 'uploadimage' }"
                    :on-success="item.handleSuccess"
                    :on-change="item.handleFileChange"
                    :before-upload="item.beforeUpload"
...
<script setup>
import TKCustomCollapse from "@/components/TKCustomCollapse/index.vue";
import TkRegionAreaPlus from "@/components/TkFormV1/TkRegionAreaPlus";
...
const props = defineProps({
  ...
  sections: {
    ...
    //     label: '基本信息',
    //     collapse: false,
    //     formTypes: ['input', 'select'], // 指定该部分包含的表单类型
    //     formProps: ['name', 'age', 'gender'] // 或者指定属性名
    //   },
    //   {
    //     type: 'advanced',
    //     label: '高级信息',
    //     collapse: true,
    //     formTypes: ['textarea', 'upload']
    //   }
    // ]
  },
  isDrawer: {
...
// 根据 section 获取对应的表单配置
const getSectionFormConfig = (section) => {
  // 如果section直接提供了formConfig
  if (section.formConfig) {
    return section.formConfig;
  }

  // 根据类型过滤表单配置
  if (section.formTypes) {
    // 保持 searchFormObj.formConfig 的原始顺序
    return props.searchFormObj.formConfig.filter((item) => section.formTypes.includes(item.type));
  }

  // 根据属性名过滤，保持原始顺序
  if (section.formProps) {
    // 创建一个映射来快速查找 formProps 中的项
    const propSet = new Set(section.formProps);

    // 按照 searchFormObj.formConfig 的顺序过滤
    return props.searchFormObj.formConfig.filter((item) => propSet.has(item.prop));
  }

  // 默认返回所有配置
  return props.searchFormObj.formConfig;
};
...
//删除老图片
function deleteOldPic(item, index) {
...
function computedImageFileList() {
  ...
  if (newImageFileLength > 0) {
    ...
    props.searchFormObj.formData.imageFileList = props.searchFormObj.formData?.oldImgFiles?.concat(
      props.searchFormObj.formData.imageFileList
    );
  } else {
    if (oldImageFileLength > 0) props.searchFormObj.formData.imageFileList = props.searchFormObj.formData.oldImgFiles;
  }
}
