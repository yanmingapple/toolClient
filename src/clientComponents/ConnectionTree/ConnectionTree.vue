<template>
  <div class="connection-tree">
    ...
    <TkContextMenu
      ...
      @menu-item-click="handleTkMenuClick"
    />
  </div>
</template>
...
import { CTMessage } from '@/utils/CTMessage'
...
const isNodeConnected = computed(() => {
  if (!contextMenuNode.value || contextMenuNode.value.type !== TreeNodeType.CONNECTION) return false
  const connectionId = contextMenuNode.value.key
...
const isDatabaseConnected = computed(() => {
  if (!contextMenuNode.value || contextMenuNode.value.type !== TreeNodeType.DATABASE) return false
  return contextMenuNode.value.children && contextMenuNode.value.children.length > 0
})

// 图标映射（转换为 SVG 图标字符串格式，TkContextMenu 使用 svg-icon）
// 根据项目中的 SVG 图标命名规范，使用 ic_ 前缀
...
const contextMenuItems = computed(() => {
    ...
    return [
      {
        label: '新建连接',
        icon: iconMap.Plus,
        handler: () => handleMenuClick('newConnection', contextMenuNode.value)
      }
    ]
  }
  ...
  switch (node.type) {
    case TreeNodeType.CONNECTION:
      // 连接节点菜单
      if (!isNodeConnected.value) {
        items.push({
          label: '打开连接',
          icon: iconMap.Connection,
          handler: () => handleMenuClick('openConnection', node)
        })
      } else {
        items.push({
          label: '断开连接',
          icon: iconMap.Remove,
          handler: () => handleMenuClick('disconnect', node)
        })
      }
      ...
      items.push({
        label: '新建连接',
        icon: iconMap.Plus,
        handler: () => handleMenuClick('newConnection', node)
      })
      items.push({
        label: '编辑连接',
        icon: iconMap.Edit,
        handler: () => handleMenuClick('editConnection', node)
      })
      items.push({
        label: '删除连接',
        icon: iconMap.Delete,
        type: 'danger',
        handler: () => handleMenuClick('deleteConnection', node)
      })
      items.push({ type: 'divider' })
      items.push({
        label: '刷新数据库',
        icon: iconMap.Refresh,
        handler: () => handleMenuClick('refreshDatabase', node)
      })
      break
    ...
    case TreeNodeType.DATABASE:
      ...
      if (!isDatabaseConnected.value) {
        items.push({
          label: '打开数据库',
          icon: iconMap.Link,
          handler: () => handleMenuClick('openDatabase', node)
        })
      } else {
        items.push({
          label: '关闭数据库',
          icon: iconMap.Remove,
          handler: () => handleMenuClick('closeDatabase', node)
        })
      }
      items.push({ type: 'divider' })
      items.push({
        label: '新建数据库',
        icon: iconMap.Plus,
        handler: () => handleMenuClick('newDatabase', node)
      })
      items.push({
        label: '新建查询',
        icon: iconMap.Document,
        handler: () => handleMenuClick('newQuery', node)
      })
      items.push({
        label: '刷新数据库',
        icon: iconMap.Refresh,
      ...
      items.push({
        ...
        icon: iconMap.Delete,
        type: 'danger',
        handler: () => handleMenuClick('deleteDatabase', node)
      })
      break
    ...
    case TreeNodeType.TABLE:
      ...
      items.push({
        label: '新建表',
        icon: iconMap.Plus,
        handler: () => handleMenuClick('newTable', node)
      })
      items.push({
        label: '新建表查询',
        icon: iconMap.Document,
        handler: () => handleMenuClick('newTableQuery', node)
      })
      items.push({ type: 'divider' })
      items.push({
        label: '查看表结构',
        icon: iconMap.InfoFilled,
        handler: () => handleMenuClick('viewStructure', node)
      })
      items.push({
        label: '查看数据',
        icon: iconMap.Grid,
        handler: () => handleMenuClick('viewData', node)
      })
      items.push({
        label: '导入数据',
        icon: iconMap.Upload,
        handler: () => handleMenuClick('importData', node)
      })
      items.push({
        label: '导出数据',
        icon: iconMap.Download,
        handler: () => handleMenuClick('exportData', node)
      })
      items.push({
        label: '删除表',
        icon: iconMap.Delete,
        type: 'danger',
        handler: () => handleMenuClick('deleteTable', node)
      })
      items.push({
        label: '刷新表',
        icon: iconMap.Refresh,
        handler: () => handleMenuClick('refreshTable', node)
      })
      break
    ...
    case TreeNodeType.VIEW:
      ...
      items.push({
        label: '新建视图',
        icon: iconMap.Plus,
        handler: () => handleMenuClick('newView', node)
      })
      items.push({
        label: '新建查询',
        icon: iconMap.Document,
      ...
      items.push({
        label: '查看视图',
        icon: iconMap.View,
        handler: () => handleMenuClick('viewView', node)
      })
      items.push({
        label: '删除视图',
        icon: iconMap.Delete,
        type: 'danger',
        handler: () => handleMenuClick('deleteView', node)
      })
      break
    ...
    case TreeNodeType.FUNCTION:
      ...
      items.push({
        label: '新建函数',
        icon: iconMap.Plus,
        handler: () => handleMenuClick('newFunction', node)
      })
      items.push({
        label: '新建查询',
        icon: iconMap.Document,
      ...
      items.push({
        label: '执行',
        icon: iconMap.VideoPlay,
        handler: () => handleMenuClick('execute', node)
      })
      items.push({
        label: '删除存储过程',
        icon: iconMap.Delete,
        type: 'danger',
        handler: () => handleMenuClick('deleteFunction', node)
      })
      break
...
// 处理 TkContextMenu 的菜单项点击
const handleTkMenuClick = (item: any) => {
  // handler 已经在菜单项配置中直接调用，这里可以处理额外的逻辑
  ...
  switch (action) {
    ...
    case 'editConnection':
      if (node.type === TreeNodeType.CONNECTION) {
        const connection = node.data?.metadata?.connection
        if (connection) {
          if (isNodeConnectedFn(connection.id)) {
            handleDisconnect(connection)
            // 折叠当前连接及其所有子节点的展开状态
            collapseConnectionNodes(connection.id)
          }
          if (props.onEditConnection) {
            // 传递整个 node 对象，因为 ConnectionDialog 需要 TreeNode 类型
            props.onEditConnection(node)
          }
    ...
    case 'newDatabase':
      if (node.type === TreeNodeType.DATABASE) {
        CTMessage.info('新建数据库功能开发中...')
      }
      break
    case 'newTable':
      if (node.type === TreeNodeType.TABLE) {
        CTMessage.info('新建表功能开发中...')
      }
      break
    case 'newView':
      if (node.type === TreeNodeType.VIEW) {
        CTMessage.info('新建视图功能开发中...')
      }
      break
    case 'newFunction':
      if (node.type === TreeNodeType.FUNCTION) {
        CTMessage.info('新建函数功能开发中...')
      }
      break
    case 'importData':
      if (node.type === TreeNodeType.TABLE) {
        CTMessage.info('导入数据功能开发中...')
      }
      break
    case 'exportData':
      if (node.type === TreeNodeType.TABLE) {
        CTMessage.info('导出数据功能开发中...')
      }
      break
    case 'refreshTable':
      if (node.type === TreeNodeType.TABLE) {
...
const isNodeConnectedFn = (connectionId: string) => {
