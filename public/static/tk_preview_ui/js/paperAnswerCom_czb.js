...
function dealContent(item) {
  ...
  } else if (item.itemTypeId == 15) { // 判断改错题 答案处理
  ...
  } else if (item.itemTypeId == 11) { // 填空题
    let ansList
  ...
  } else if (item.itemTypeId == 16) { // 填空题
...
function doPaperHtmlStr(data, type, showPaperItemNumData) {
  ...
  forFun(data, function (_p, index) {
    ...
    if (_p.menuInfoType == 1) { // 结构
      if (_p.type === 2) {
        ...
        if (structureIsItemBlock) {
          paperHtmlStr += `<div data-op-type="${structDataopType}" class="structure-con structContent mceNonEditable block-item"><span>${_p.name}：</span><span class="${itemTypeMap['999']}">${_p.description || ''}</span></div>`
        } else {
          paperHtmlStr += `<div data-op-type="${structDataopType}" class="structure-con structContent"><span>${_p.name}：</span><span class="${itemTypeMap['999']}">${_p.description || ''}</span></div></div>`
      ...
      if (type == 'item' && ![7].includes(_p.itemTypeId * 1)) {
        if ([16].includes(_p.itemTypeId * 1)) {
          ...
          paperHtmlStr += `<div class="item-con f-item-con item-con-of-type-${_p.itemTypeId} ${isRowItemsCls}">${prevHtmlStr + contentHtmlStr}</div>` // block-item 
        } else {
          prevHtmlStr = _p.paperItemNum ? `<div class="stem-prev-con f-prev-con"><span>${_p.paperItemNum}</span><span class="cn-dot-con">.</span></div>` : ''
          let contentStr = dealContent(_p)
          contentHtmlStr = contentStr ? `<div class="stem-con-content f-content-con">${contentStr}</div>` : ''

          // 填空题答案超出一半则占一整行
          let ansClsOfFillblank = ''
          if(_p.itemTypeId == 11) {
            let $tempBox = $('#tempBox')
            ...
            let fillAnsWidth = $tempBox.html(`<div class="item-con f-item-con item-con-of-type-${_p.itemTypeId} ${isRowItemsCls}">${prevHtmlStr + contentHtmlStr}</div>`).outerWidth(true)
            ansClsOfFillblank = fillAnsWidth > contentW/2 ? 'whole-row' : ''
          }

          if (prevHtmlStr || contentHtmlStr) {
            paperHtmlStr += `<div class="item-con f-item-con item-con-of-type-${_p.itemTypeId} ${isRowItemsCls} ${ansClsOfFillblank}">${prevHtmlStr + contentHtmlStr}</div>` // block-item 
          }
        }
      }
      if (type == 'childItem') {
        ...
        paperHtmlStr += `<div class="item-con s-item-con item-con-of-type-${_p.itemTypeId} ${isRowItemsCls}">${prevHtmlStr + contentHtmlStr}</div>` // block-item 
      }
    }
    ...
    if (_p.childrenPaperPriewVOs && _p.childrenPaperPriewVOs.length > 0) {
      ...
      if (itemTypeId) isColItems = [1, 2, 3, 4, 11, 12, 13, 14].includes(itemTypeId * 1) // 答案不独行显示
      ...
      if (itemTypeId) isColItems = [1, 2, 3, 4, 13, 14].includes(itemTypeId * 1) // 答案不独行显示

      // 如果综合体语料有题号
      if (_p.paperItemNum) {
        showPaperItemNum = '<div class="stem-prev-con parent-papernum-of-childitem"><span>' + _p.paperItemNum + '</span><span class="cn-dot-con">.</span></div><div class="stem-prev-con s-prev-con">' + innerData[0].paperItemNum + '</div>'
      }
    }

    if (isColItems) {
      // 综合题不加 col-block-item，防止同行显示
      paperHtmlStr += `<div class="block-item ${_p.itemTypeId ? '' : 'mceNonEditable col-block-item'}">${doPaperHtmlStr(innerData, innerType, showPaperItemNum)}</div>`
    } else paperHtmlStr += doPaperHtmlStr(innerData, innerType, showPaperItemNum)
  })
...
function dealPaperAnswerDataOfcalcCon(callback) {
  //后续需要判断科目头部如何取值
  var paperHtmlStr = '<div class="mceNonEditable block-item"><div class="paperName">' + paperName + '</div>'
  //试卷内容数据
...
// 页脚内容设置
function setFooter() {
  bookConfig['simplePageNum']['pendant'] = '<div style="padding-bottom: 34mm;left: 0px;width: 100%;height: 4cm; " class="page-num-simple"><span style=\"color: #000;\">' + paperName + '参考答案第${PAGE}页（共${TOTAL_PAGE}页）</span></div>'
}
