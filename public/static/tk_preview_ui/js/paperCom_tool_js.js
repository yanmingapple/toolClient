var paperId = getQueryString('paperId'),
  ...
  // isSystem=assignTool ip port  命题辅助工具，带有ip和port，获取接口的ip和port使用传入的
  ...
  subjectName = getQueryString('subjectName'), // && unescape(getQueryString('subjectName')) || ''
  ...
  warmTipField = 'common', // 卷首提示语字段
  paperTitleField = 'otherExam', // 卷首卷名字段
  retdata = [], // 试卷数据
  paperInfo = {},
  examYear = '&nbsp;&nbsp;&nbsp;&nbsp;',
  examMonth = '&nbsp;&nbsp;&nbsp;&nbsp;',
  checkRecord = [], // 审校人员记录数据
  specialText = '', // 特殊考试要求数据
  subjectData = {}, // 题型配置
  // 1 单项选择题 single
...
function getPaperTitle() {
  var warmTipsMap = {
      common:
        '<p>1．本试卷分为两部分，第一部分为选择题，第二部分为非选择题。</p>' +
        '<p>2．应考者必须按试题顺序在答题卡（纸）指定位置上作答，答在试卷上无效。</p>' +
        '<p>3．涂写部分、画图部分必须使用2B铅笔，书写部分必须使用黑色字迹签字笔。</p>',
      SL:
        '<p>1．应考者必须按试题顺序在答题卡（纸）指定位置上作答，答在试卷上无效。</p>' +
        '<p>2．涂写部分、画图部分必须使用2B铅笔，书写部分必须使用黑色字迹签字笔。</p>',
      noPart:
        '<p>1．应考者必须按试题顺序在答题卡（纸）指定位置上作答，答在试卷上无效。</p>' +
        '<p>2．涂写部分、画图部分必须使用2B铅笔，书写部分必须使用黑色字迹签字笔。</p>',
    },
    firstPaperNodeData = (retdata && retdata[0]) || {},
    // countryExam: 全国卷卷首  otherExam: 其他试卷
    paperTitle = {
      otherExam:
        '<div class="block-item mceNonEditable">' +
        '<p class="paper-info">绝密★启用前</p>' +
        '<div class="paperName">' +
        examYear +
        '年' +
        examMonth +
  ...
  return paperTitle[paperTitleField];
}
  ...
  forFun($blockDom, function (item) {
    ...
    for (let i = 0; i < $figcaption.length; i++) {
      ...
      if ($($figcaption[i]).text() == '题xxx图') {
        let captionText =
          $figcaption.length == 1 ? '题' + paperItemNum + '图' : '题' + paperItemNum + '-' + (i + 1) + '图';
...
function dealPaperDataOfcalcCon(callback) {
  ...
  //科目头部取值
  warmTipField = isSL ? 'SL' : isNoPart ? 'noPart' : 'common';
  var paperHtmlStr = getPaperTitle();
  function setPaperData() {
    ...
    dealSpecialContnt();
    removeStyle(
      $('.calc-con').find('*').not('table, thead, tbody, tr, th, td, tfooter, .blank, .stem-prev-con, .ignoreCls'),
    );
    ...
    if (_p.menuInfoType == 1) {
      ...
      // 结构
        ...
        if (!isSL) {
          ...
          if (lastDataIsPart) {
            paperHtmlStr +=
              '<div class="structure-con structContent"><span>' + _p.name + '</span>' + descHtmlStr + '</div></div>';
            lastDataIsPart = false;
          } else {
            paperHtmlStr +=
              '<div data-op-type="' +
              structDataopType +
              '" class="structure-con block-item structContent mceNonEditable"><span>' +
              _p.name +
              '</span>' +
              descHtmlStr +
              '</div>';
      ...
      if (!$tempDom.text() && $imgDoms && $imgDoms.length == 0) {
        ...
        var titlePreXH =
          _p.paperItemNum && !isSL
            ? '<span class="' +
              (index == 0 && showPaperItemNumData ? 'papernumofchilditem' : '') +
              '">' +
              ((index == 0 && showPaperItemNumData) || _p.paperItemNum) +
              '</span>' +
              (_p.paperItemNum.indexOf(')') >= 0 ? '' : '<span class="cn-dot-con">.</span>')
            : '';
        ...
        _p.itemContent =
          $(`<div>${_p.itemContent}</div>`).find('p').length > 0 ? _p.itemContent : `<p>${_p.itemContent}</p>`;
      ...
      var optionHtmlStr =
        ...
        _p.name +
        '<span class="cn-dot-con">.</span>&nbsp</div><div>' +
        _p.content +
        '</div></div>';
      ...
      if (
        ...
        if (_p.paperItemNum) {
          showPaperItemNum =
            ...
            _p.paperItemNum +
            '</span><span class="cn-dot-con">.</span></div>' +
...
function getPaperData(paperDataCallback, paperHtmlCallback, ret) {
  if (ret) {
    if (
      location.pathname == '/tk_preview/paperViewEditor.html' &&
      !subjectCode &&
      ret.publicPaper &&
      ...
      ret.publicPaper.forEach(_r => {
        window.open(
          `/tk_preview/paperViewEditor.html?paperId=${paperId}&stage=${stage}&activityId=${activityId}&subjectCode=${
            _r.subjectCode
    ...
    retdata = ret.paper || [];
    checkRecord = ret.checkRecord || [];
    specialText = ret.specialText || '';
    ...
    if (isSystem == 'assignTool') {
      document.title = subjectData.paperCode;
    } else {
      let prevStr = '';
      if (subjectCode) {
        prevStr = '共卷-';
      ...
      document.title =
        prevStr + subjectData.subjectCode + '-' + subjectData.subjectName + '-' + subjectData.paperCode + '-试卷';
    }
    //是否有单元,与申论有冲突，有限申论
    if (retdata[0] && retdata[0].subjectCode == '05006') {
      isSL = true;
    } else {
      if (retdata[0] && retdata[0].menuInfoType == 1 && retdata[0].type != 1) {
        isNoPart = true;
      }
    }
    // 全国卷卷首 key 值设置
    if (ret && ret.examType) {
      paperTitleField = ret.examType == 1 ? 'countryExam' : 'otherExam';
    }
    let paperHtmlStr = '',
    ...
    if (ret.paperHtml && paperHtmlCallback && paperHtmlCallback instanceof Function && dataType != 'origin') {
      if (isSystem == 'assignTool') {
        // 命题工具保存前端base64加密，取值需要解密
        try {
          paperHtmlStr = utf8To16($.base64.decode(paperHtmlStr, 'utf-8'));
...
function addSpecialText() {
  if (specialText) {
    return `<div class="specialTextCon" style="font-family: 仿宋;">（${specialText}）</div>`;
  } else return '';
}
...
function dealImg() {
  ...
  if (imgArr.length == 0) {
    ...
    removeStyle(
      $('.paper-con').find('*').not('table, thead, tbody, tr, th, td, tfooter, .blank, .stem-prev-con, .ignoreCls'),
    );
    d1.resolve();
  } else {
...
//     let replaceStr = $(dom).text().replaceAll('“', '<span class="cn-dot-con">$&</span>')
//       .replaceAll('”', '<span class="cn-dot-con">$&</span>')
//       .replaceAll('：', '<span class="cn-dot-con">$&</span>')
//       // .replace('.', '<span class="cn-dot-con">$&</span>')
//       .replaceAll('，', '<span class="cn-dot-con">$&</span>')
//       .replaceAll('、', '<span class="cn-dot-con">$&</span>')
//       .replaceAll('·', '<span class="cn-dot-con">$&</span>')
