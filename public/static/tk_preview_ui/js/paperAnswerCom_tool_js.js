var paperId = getQueryString('paperId'),
  ...
  paperCode = getQueryString('paperCode'),
  publicPaperId = getQueryString('publicPaperId'),
  ...
  forFun($blockDom, function (item) {
    ...
    for (let i = 0; i < $figcaption.length; i++) {
      ...
      if ($($figcaption[i]).text() == '题xxx图') {
        let captionText =
          $figcaption.length == 1 ? '答' + paperItemNum + '图' : '答' + paperItemNum + '-' + (i + 1) + '图';
...
function setDotOfPaper() {
  let $itemCons = $('.item-con');
  for (var i = 0; i < $itemCons.length; i++) {
    let d = doAllDot($itemCons[i]);
    $($itemCons[i]).replaceWith(d);
  }
}
function doAllDot(dom) {
  if (dom.nodeName === '#text') {
    ...
    let $childDoms = dom.childNodes,
      $tempDom = $(dom).clone(true);
    for (var i = 0; i < $childDoms.length; i++) {
      let charStr = this.doAllDot($childDoms[i]);
      if (i === 0) {
        $tempDom.html(charStr);
      } else {
        $tempDom.append(charStr);
      }
    }
    return $tempDom[0].outerHTML;
  }
}
// // 如果试卷标题没有年月，则设置其年月
// function setDate() {
...
function dealContent(item) {
  ...
  if (item.itemTypeId == 3) {
    ...
    else content = content == 'right' ? '√。' : '×。' + content;
    content = `<div class="judge-correct-ans-con">${content}</div>`;
  ...
  } else if (item.itemTypeId == 16) {
    // 填空题

    if (content && content.indexOf('_$_$') > -1) {
      var ss = content.split('_$_$');
      let ansList = [];
      for (var i = 0, l = ss.length; i < l; i++) {
        ...
        let innerCon = ss[i],
          ...
          $pDom = $sDom.find('p');
        if ($pDom.length > 0) {
          $pDom.each((index, item) => {
            $pDom.contents().unwrap().wrap('span').append('</br>');
          });
          innerCon = $sDom.html();
        }
        ansList.push(item.paperItemNum * 1 + i + '. ' + innerCon);
      }
      content = ansList.join(' ');
    } else {
      let ansList;
      if (item.attributeList && item.attributeList instanceof Array && item.attributeList.length > 0) {
        ansList = item.attributeList.filter(_a => _a.type == 'answer');
...
function setFooter() {
  bookConfig['simplePageNum']['pendant'] =
    '<div style="padding-bottom: 34mm;left: 0px;width: 100%;height: 4cm; " class="page-num-simple"><span style="color: #000;">' +
    paperInfo.subject +
    '参考答案第${PAGE}页（共${TOTAL_PAGE}页）</span></div>';
}
 ...
 */
let structureIsItemBlock = false,
  curPart = 0;
function doPaperHtmlStr(data, type, showPaperItemNumData, nodeIsColItems) {
  //定义试卷处理后的html
  var paperHtmlStr = '';
  ...
  forFun(data, function (_p, index) {
    ...
    if (_p.menuInfoType == 1) {
      ...
      if (_p.type === 2) {
        if (!isSL) {
          // 答案结构上的描述 用 。 分割，展示分值部分
          if (_p.description) {
            _p.description = _p.description.substr(0, _p.description.indexOf('。') + 1) || _p.description;
          }
          if (structureIsItemBlock) {
            paperHtmlStr += `<div data-op-type="${structDataopType}" class="structure-con structContent mceNonEditable block-item"><span>${
              _p.name
            }：</span><span class="${itemTypeMap['999']}">${_p.description || ''}</span></div>`;
          } else {
            paperHtmlStr += `<div data-op-type="${structDataopType}" class="structure-con structContent"><span>${
              _p.name
            }：</span><span class="${itemTypeMap['999']}">${_p.description || ''}</span></div></div>`;
            structureIsItemBlock = true;
          }
        } else {
          paperHtmlStr += '</div>';
        }
      ...
      // 1 单选  2多选 3判断 4不定项 7 综合题  8问答题 11 填空  12完型填空  13 填空选择  14 双项选择  15 判断改错
      // 综合体语料部分不显示答案,非综合体直接显示答案
      let prevHtmlStr = '',
        contentHtmlStr = '',
        isRowItemsCls = nodeIsColItems ? '' : 'block-item';
      // , isRowItemsCls = [8, 15, 16].includes(_p.itemTypeId * 1) ? 'block-item' : '' // 答案不独行显示
      if (type == 'item' && ![7].includes(_p.itemTypeId * 1)) {
        if ([16].includes(_p.itemTypeId * 1)) {
          prevHtmlStr = ''; //`<div class="stem-prev-con f-prev-con"></div>`
          contentHtmlStr = `<div class="stem-con-content f-content-con">${dealContent(_p)}</div>`;
          if (isSL) prevHtmlStr = '';
          paperHtmlStr += `<div class="item-con f-item-con item-con-of-type-${_p.itemTypeId} ${isRowItemsCls}">${
            prevHtmlStr + contentHtmlStr
          }</div>`; // block-item
        } else {
          prevHtmlStr = _p.paperItemNum
            ? `<div class="stem-prev-con f-prev-con"><span>${_p.paperItemNum}</span><span class="cn-dot-con">.</span></div>`
            : '';
          ...
          // 填空题答案超出一半则占一整行
          let ansClsOfFillblank = 'whole-row';
          if (_p.itemTypeId == 11 && isCountryExam) {
            let $tempBox = $('#tempBox');
            if (!$tempBox || !$tempBox.length) {
              $tempBox = $('<div id="tempBox" style="display: inline-block;"></div>');
              $('body').append($tempBox);
            }
            let fillAnsWidth = $tempBox
              .html(
                `<div class="item-con f-item-con item-con-of-type-${_p.itemTypeId} ${isRowItemsCls}">${
                  prevHtmlStr + contentHtmlStr
                }</div>`,
              )
              .outerWidth(true);
            ansClsOfFillblank = fillAnsWidth > contentW / 2 ? 'whole-row' : '';
          }

          if (prevHtmlStr || contentHtmlStr) {
            paperHtmlStr += `<div class="item-con f-item-con item-con-of-type-${
              _p.itemTypeId
            } ${isRowItemsCls} ${ansClsOfFillblank}">${prevHtmlStr + contentHtmlStr}</div>`; // block-item
          }
        }
      }
      if (type == 'childItem') {
        if (showPaperItemNumData && index == 0) {
          ...
          let dotStr = _p.paperItemNum.indexOf('(') > -1 ? '' : '<span class="cn-dot-con">.</span>',
            tempCls = showPaperItemNumData && index !== 0 ? 'next-child-ans-prev' : '';
          prevHtmlStr = `<div class="stem-prev-con s-prev-con ${tempCls}"><span>${_p.paperItemNum}</span>${dotStr}</div>`;
        }
        contentHtmlStr = `<div class="stem-con-content s-content-con">${dealContent(_p)}</div>`;
        if (isSL) prevHtmlStr = '';
        paperHtmlStr += `<div class="item-con s-item-con item-con-of-type-${_p.itemTypeId} ${isRowItemsCls}">${
          prevHtmlStr + contentHtmlStr
        }</div>`; // block-item
      }
    }
    ...
    if (_p.childrenPaperPriewVOs && _p.childrenPaperPriewVOs.length > 0) {
      ...
      // paperHtmlStr += '<div class="item-types-con">'
      innerData = _p.itemList;
      innerType = 'item';
      var itemTypeId = (_p.itemList && _p.itemList[0] && _p.itemList[0].itemTypeId) || '';
      if (itemTypeId && (curPart == 0 || isCountryExam))
        isColItems = [1, 2, 3, 4, 11, 12, 13, 14].includes(itemTypeId * 1); // 答案不独行显示
      // console.log(curPart, _p.name)  curPart  isCountryExam
    } else if (_p.childItemList && _p.childItemList.length > 0) {
      innerData = _p.childItemList;
      innerType = 'childItem';
      var itemTypeId = (_p.childItemList && _p.childItemList[0] && _p.childItemList[0].itemTypeId) || '';
      if (itemTypeId && (curPart == 0 || isCountryExam)) isColItems = [1, 2, 3, 4, 13, 14].includes(itemTypeId * 1); // 答案不独行显示
      // 如果综合体语料有题号
      if (_p.paperItemNum) {
        showPaperItemNum =
          '<div class="stem-prev-con parent-papernum-of-childitem"><span>' +
          _p.paperItemNum +
          '</span><span class="cn-dot-con">.</span></div><div class="stem-prev-con s-prev-con">' +
    ...
    if (isColItems) {
      // 综合题不加 col-block-item，防止同行显示
      paperHtmlStr += `<div class="block-item ${_p.itemTypeId ? '' : 'mceNonEditable col-block-item'}">${doPaperHtmlStr(
        innerData,
        innerType,
        showPaperItemNum,
        isColItems,
      )}</div>`;
    } else paperHtmlStr += doPaperHtmlStr(innerData, innerType, showPaperItemNum, isColItems);
  });
...
function dealPaperAnswerDataOfcalcCon(callback) {
  // 答案标题是否缩小字号
  let smallerSizeCls = '';
  if (subjectData.subjectName && subjectData.subjectName.length >= 13) smallerSizeCls = ' m_smaller';
  else if (subjectData.subjectName && subjectData.subjectName.length > 8) smallerSizeCls = ' smaller';

  isSL = subjectData.subjectCode == '05006';
  var paperTitle = {
    otherExam:
      '<div class="mceNonEditable block-item"><p class="paper-info">绝密★启用前</p>' +
      '<div class="paperName">' +
      examYear +
      '年' +
      examMonth +
  ...
  //后续需要判断科目头部如何取值
  var paperHtmlStr = paperTitle[paperTitleField];
  //试卷内容数据
  paperHtmlStr += doPaperHtmlStr(retdata);
...
function getPaperData(answerDataCallback, answerHtmlCallback, ret) {
  if (ret) {
    if (
      location.pathname == '/tk_preview/paperAnswerViewEditor.html' &&
      !subjectCode &&
      ret.publicPaper &&
      ret.publicPaper.length > 0
    ) {
      ret.publicPaper.forEach(_r => {
        window.open(
          `/tk_preview/paperAnswerViewEditor.html?paperId=${paperId}&stage=${stage}&activityId=${activityId}&subjectCode=${
            _r.subjectCode
          }&subjectName=${escape(_r.subjectName)}&paperCode=${_r.name}&publicPaperId=${_r.id}`,
        );
      });
    }
    paperInfo['subject'] = ret.subject || '';
    retdata = ret.menuList || ret.paper || [];
    examYear = ret.examYear || examYear;
    examMonth = ret.examMonth ? Number(ret.examMonth) : examMonth;
    isCountryExam = ret.examType == 1;
    paperTitleField = isCountryExam ? 'countryExam' : 'otherExam';

    subjectData.subjectName = subjectName || ret.subjectName || (retdata && retdata[0] && retdata[0].subjectName) || '';
    subjectData.subjectCode = subjectCode || ret.subjectCode || (retdata && retdata[0] && retdata[0].subjectCode) || '';
    subjectData.paperCode = paperCode || ret.paperCode || (retdata && retdata[0] && retdata[0].paperName) || '';
    // 网页标题设置，辅助工具和tk系统区分
    if (isSystem == 'assignTool') {
      document.title = subjectData.paperCode + '答案';
    } else {
      let prevStr = '';
      if (subjectCode) {
        prevStr = '共卷-';
      ...
      document.title =
        prevStr + subjectData.subjectCode + '-' + subjectData.subjectName + '-' + subjectData.paperCode + '答案';
    }
    let answerHtml = '';
    ...
    if (answerHtml && answerHtmlCallback && answerHtmlCallback instanceof Function && dataType != 'origin') {
      ...
      //   } catch (error) {}
      // } else if(ret.answerEditVersion && ret.answerEditVersion == 2) {
      //   answerHtml = utf8To16($.base64.decode(answerHtml, "utf-8"))
      // }
