...
var DEFAULT_FONT = '../fonts/SimSun.ttf';
...
class TextToSVG {
    ...
    //网页加载
    static load(url, key, cb) {
        opentype.load(url, (err, font) => {
            if (err !== null) {
                return cb(key, null);
            }

            return cb(key, new TextToSVG(font));
        });
    }
    ...
    getWidth(text, options) {
        ...
        for (let i = 0; i < glyphs.length; i++) {
            ...
            // 计算letterSpacing或tracking
            let extraSpacing = 0;
            if (options.letterSpacing) {
                extraSpacing = (glyphs.length - 1) * options.letterSpacing * fontSize;
            } else if (options.tracking) {
                extraSpacing = (glyphs.length - 1) * (options.tracking / 1000) * fontSize;
            }
            width += extraSpacing;
        }
        return width;
    ...
    getD(text, options = {}) {
        const fontSize = options.fontSize || 72;
        const kerning = 'kerning' in options ? options.kerning : true;//字间距
        const letterSpacing = 'letterSpacing' in options ? options.letterSpacing : false;//子间隔空
        const tracking = 'tracking' in options ? options.tracking : false;//跟踪
        
        const metrics = this.getMetrics(text, options);
        const path = this.font.getPath(text, metrics.x, metrics.baseline, fontSize , {
            features: { liga: true },// 启用连字
            kerning, 
            letterSpacing,
            tracking,
        });
        return path.toSVG(7);
        // return path.toPathData(7);// 0表示无小数,1表示一位，2表示二位
    }

    getPath(text, options = {}) {
        // 使用charToGlyphIndex探查字形
        const glyphIndex = this.font.charToGlyphIndex(text);
