...
(function () {
    ...
    var firePasteAfterProcess = function (editor, html, internal) {
      return editor.fire('PasteAfterProcess', {
        content: html,
        internal: internal
      });
    };
    ...
    var getAfterProcess = function (editor) {
      return editor.getParam('paste_afterprocess');
    };
    ...
    var doPaste = function (editor, content, internal, pasteAsText) {
      ...
      firePasteAfterProcess(editor, content, internal);
    };
    ...
    var getImageSize = function (url) {
      return new global$8(function (callback) {
        ...
        img.onload = function () {
          ...
          done(global$8.resolve(dimensions));
        };
        img.onerror = function () {
          done(global$8.reject('Failed to get image dimensions for: ' + url));
        };
        var style = img.style;
    ...
    var pasteImage = function (editor, imageItem) {
      ...
      var id = uniqueId();
      var blob = imageItem.blob;
      var blobInfo = createBlobCache(editor,blob, id, imageItem.uri);
        return global$imageUploader(editor).upload([blobInfo], false).then(function (results) {
          if (results.length === 0) {
            ...
            getImageSize(results[0].url).then(function(dimensions){
              editor.insertContent(`<img data-dpi="96" style="width:${dimensions.width}px;height:${dimensions.height}px;" src="${results[0].url}" </img>`)
            })
            return results[0];
          }
        });
    ...
    var setup$1 = function (editor) {
      ...
      var afterProcess = getAfterProcess(editor);
      if (afterProcess) {
        editor.on('PasteAfterProcess', function (e) {
          afterProcess.call(plugin,editor, e);
        });
      }
    ...
    var getFormats = function (editor) {
      return editor.getParam('paste_model', [
        'text',
        'html',
      ]);
    };
    ...
    var getDefaultFillBlankValue = function (editor) {
      ...
      return formats.length > 0 ? formats[0] : 'text';
    };

    var getPasteModelDesc = function (editor, fmt) {
      debugger
      var ret = '';
      if (fmt === 'text') {
        ret = '文本'
    ...
    var register = function (editor, clipboard) {
      ...
      editor.ui.registry.addSplitButton('pastetext', {
        icon: 'paste-text',
        tooltip: 'Paste as text',
        select: function (value) {
          return value === clipboard.pasteFormat.get();
        },
        fetch: function (done) {
          done( global$6.map(formats, function (format) {
            return {
              type: 'choiceitem',
              text: getPasteModelDesc(editor, format),
              value: format
            };
          }));
        },
        ...
        onItemAction: function (_api, value) {
          clipboard.pasteFormat.set(value);
        }
      });
