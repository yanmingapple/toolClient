...
(function () {
	...
	EditorUi.enableLogging = urlParams['stealth'] != '1' && urlParams['lockdown'] != '1' &&  // yanwx
	...
	EditorUi.prototype.fileLoaded = function (file, noDialogs) {
		console.log('diagramly-EditorUi.fileLoaded()');
	...
	EditorUi.prototype.showImageDialog = function (title, value, fn, ignoreExisting, convertDataUri) {
		console.log('diagramly-EditorUi.showImageDialog()');
	...
	EditorUi.prototype.handleError = function (resp, title, fn, invokeFnOnClose, notFoundMessage, fileHash, disableLogging) {
		...
		if (e != null || title != null) {
			...
			if (e != null && e.helpLink != null && (urlParams['customMenu'] != '1'))  // yanwx,定制菜单
			{
	...
	EditorUi.prototype.createImageDataUri = function (canvas, xml, format, dpi) {
		var data = canvas.toDataURL('image/' + format,1.3);
	...
	EditorUi.prototype.saveCanvas = function (canvas, xml, format, ignorePageName, dpi) {
		...
		// console.log('diagramly-EditorUi.saveCanvas', data);
	...
	/**
	 * js2exe_SaveToAnswerArea:保存答案
	 * @isSaveAndQuit，false仅保存答案, true:保存答案并退出app；
	 * @nSaveMode, [0为js2exe定时器保存，1为js2exe点击保存，2为exe2js调用保存];
	 */
	EditorUi.prototype.js2exe_SaveToAnswerArea = function (callbackFun) {
		try {
			
			this.editor.exportToCanvas(mxUtils.bind(this, function (canvas) {
				var svgObj = this.editor.graph.getSvg()

				 
				var width = canvas.width ;
				var height = canvas.height;
			
				svgObj.setAttribute("width", width + "px"),
				svgObj.setAttribute("height", height+"px");
				debugger
				var n = svgObj.outerHTML
				, t = new Image;
				t.width = width,
				t.height  = height;
				  ...
				  fileReader.onload = (e) => {
					...
					var bIsUncompressXmlData = false;	// 是否压缩XML数据
					var svgCodeByXml = Graph.xmlDeclaration + '\n' +
					window.editorUi.getFileData(true, null, null, null, true, false, null, null, null, bIsUncompressXmlData);


					if (callbackFun) callbackFun(ImgBase64, svgCodeByXml);
				...
				debugger
				// 异步获取base64
				// var svgImgB64 = this.createImageDataUri(canvas, null, 'png');
				var svgImgB64 = this.createImageDataUri(canvas, null, 'png',10);
				//console.log('diagramly-EditorUi.exportToCanvas():', svgImgB64);
				...
				// 203;{"FuncID":203, "FuncName":"svgSaveAnswerData"
				// 数据格式："203;{isQuit}false{/isQuit},{saveStatus}3{/saveStatus},{saveMode}0{/saveMode},{hytSvgImg}data:image/jpeg;base64,iVBORw0KGgo.....==={/hytSvgImg},\r\n{hytSvgCode}svgBase64....{/hytSvgCode}" 

				//console.log("--- diagramly-EditorUi.js2exe_SaveToAnswerArea(), szReqData:", szReqData);

				if (callbackFun) callbackFun(svgImgB64, svgCodeByXml);

			}), 780);  // yanwx, 导出png最宽为800px;
		}
		catch (e) {
			this.handleError(e);
		}
	 ...
	 * 重新加载“预置作答”
	 */
	EditorUi.prototype.exe2js_LoadDataFromExe = function (szSvgXmlCode) {
		// 加载SVG
		...
		try {
			...
			// 设置图层
			let children = this.editor.graph.model.root.children, activeIndex;
			...
			if (activeIndex) {
				this.editor.graph.setDefaultParent(this.editor.graph.model.getChildAt(this.editor.graph.model.root, activeIndex));
			}
	...
	EditorUi.prototype.openKformula = function () {
	...
	EditorUi.prototype.importFiles = function (files, x, y, maxSize, fn, resultFn, filterFn, barrierFn,
		...
		console.log('diagramly-EditorUi.importFiles()');
	...
	EditorUi.prototype.loadImage = function (uri, onload, onerror) {
		console.log('diagramly-EditorUi.loadImage()');
		try {
			var img = new Image();
			img.onload = function () {
	...
	EditorUi.prototype.initFormula = function () {
		var graph = this.editor.graph;
		var ui = this;
		var factory = kf.EditorFactory.create($("#kfEditorContainer")[0], {
			...
			resource: {
				path: "./kityformula/kityformula/resource/"
			}
		});

		factory.ready(function (KFEditor) {
			var nSaveAnswerTimerInterval = urlParams['saveTimerInterval'];  // 秒
			if (0 >= nSaveAnswerTimerInterval) {
				// 小于等于0，不定时保存作答
			}
			else {
				// 定时20秒保存作答
				nSaveAnswerTimerInterval = nSaveAnswerTimerInterval * 1000;  // 毫秒
				if (20000 > nSaveAnswerTimerInterval) {
					nSaveAnswerTimerInterval = 20000;
				}

				setInterval(() => {
					// 每60秒定时保存答案
					window.editorUi.js2exe_SaveToAnswerArea(false, 0);   // yanwx, 0：定时保存, js2exe
				}, nSaveAnswerTimerInterval);
			}

			$("#tips").remove();
			window.kfe = this;
			console.log('diagramly-EditorUi.Formula.ready()');
			/****
			 * // 取消提示
			var initPageCell = new mxCell('提示：作图区', new mxGeometry(0, 1, 100, 30), "text;whiteSpace=wrap;html=1;fontColor=#ff0000;fontSize=14;")
			initPageCell.vertex = true;
			var pt = graph.getCenterInsertPoint(graph.getBoundingBoxFromGeometry([initPageCell], true));
			pt.x = 100; pt.y = 100;
			initPageCell.geometry.x = pt.x;
			initPageCell.geometry.y = pt.y;
			graph.addCell(initPageCell);
			****/

			if (window.kfe) {
				$('#kfEditorContainer').append($('<div class="closeDiv">取消</div>'))
				$('#kfEditorContainer').append($('<div class="savaDiv">确认</div>'))
				window.kfe.execCommand("render", null || "\\placeholder");

				$('.closeDiv').click(() => {
					$('.kf-model').css('visibility', 'hidden')
				});

				$('.savaDiv').click(() => {
					let isNull = checkFormulaIsNull()
					// 为空不保存
					if (isNull) {
						$('.kf-model').css('visibility', 'hidden')
						return
					}
					let selectEle = graph.getSelectionCells()[0]
					let cell = setImg(selectEle)
					console.log(cell)
					if (selectEle && selectEle.latex) {
						console.log('编辑')
					} else {
						console.log('创建')
						graph.addCell(cell);
				...
				$('.closeKfIcon').click((e) => {
					...
					//(msg, okFn, cancelFn, okLabel, cancelLabel, closable)
					ui.confirm('请确认是否保存', function () {
						let selectEle = graph.getSelectionCells()[0];
						let cell = setImg(selectEle);
						console.log(cell);
						if (selectEle && selectEle.latex) {
							console.log('diagramly-EditorUi.Formula-close编辑');
						} else {
							console.log('diagramly-EditorUi.Formula-close创建');
							graph.addCell(cell);
						}
				...
				$('.kf-save').click(function (event) {
					...
					console.log(cell);
					if (selectEle && selectEle.latex) {
						console.log('diagramly-EditorUi.Formula-save编辑');
					} else {
						console.log('diagramly-EditorUi.Formula-save创建');
						graph.addCell(cell);
					}
				...
				// 窗口移动
				$('.kf-header').mousedown(function (e) {
					// e.pageX
					var positionDiv = $('.kf-header-container').offset();
					...
					$(document).mousemove(function (e) {
						...
						if (x < 0) {
							x = 0;
						} else if (x > $(document).width() - $('.kf-header-container').outerWidth(true)) {
							x = $(document).width() - $('.kf-header-container').outerWidth(true);
						}

						if (y < 0) {
							y = 0;
						} else if (y > $(document).height() - $('.kf-header-container').outerHeight(true)) {
							y = $(document).height() - $('.kf-header-container').outerHeight(true);
						}

						$('#kfEditorContainer').css({
							'left': x + 'px',
							'top': y + 'px'
						});
					});
				...
				$('.special-sharacters-header').mousedown(function (e) {
					// e.pageX
					var positionDiv = $('.special-sharacters-header').offset();
					var distenceX = e.pageX - positionDiv.left;
					var distenceY = e.pageY - positionDiv.top;
					...
					$(document).mousemove(function (e) {
						...
						if (x < 0) {
							x = 0;
						} else if (x > $(document).width() - $('.special-sharacters-header').outerWidth(true)) {
							x = $(document).width() - $('.special-sharacters-header').outerWidth(true);
						}

						if (y < 0) {
							y = 0;
						} else if (y > $(document).height() - $('.special-sharacters-header').outerHeight(true)) {
							y = $(document).height() - $('.special-sharacters-header').outerHeight(true);
						}

						$('.special-sharacters-container').css({
							'left': x + 'px',
							'top': y + 'px'
						});
					});
				...
				function checkFormulaIsNull() {
					let textEle = $('text[text-rendering="geometricPrecision"]');
					let isNull = false;
					for (let i = 0; i < textEle.length; i++) {
						const item = textEle[i];
						if (item.innerHTML == '在此处键入公式') {
							isNull = true;
						}
					}
					return isNull;
				...
				function setImg(cell) {
					var width = document.getElementById('kity_g_2').getBoundingClientRect().width;
					var height = document.getElementById('kity_g_2').getBoundingClientRect().height + 20;
					if (!cell) {
						cell = new mxCell('', new mxGeometry(0, 1, width, height), "text;whiteSpace=wrap;html=1;fontColor=#000000;fontSize=14;")
						cell.vertex = true;
					...
					graph.getModel().beginUpdate()
					// 清除公式空白占位符 或者 不给保存
					let latex = window.kfe.execCommand('get.source').replaceAll('\\placeholder', '\{(~)}')
					// let latex = window.kfe.execCommand('get.source')
					cell.value = `$$\\ ${latex}$$`;
					cell.latex = latex;
					...
					// 直接克隆 svg 转 base64 png 显示
					var svgroot = document.createElementNS("http://www.w3.org/2000/svg", "svg");
					let ele = $('.kf-editor-canvas-container svg').children()
					for (let i = 0; i < ele.length; i++) {
						$(svgroot).append($(ele[i])[0].cloneNode(true))
					}
					// 去掉光标
					$(svgroot).attr('transform', '').find("#kity_path_5").remove();
					$(svgroot).find('#kity_g_3').attr('transform', 'matrix( 0.48 0 0 0.48 0 0 )')
					var s = new XMLSerializer().serializeToString(svgroot)
					var encodedData = window.btoa(unescape(encodeURIComponent(s)));
					var newValue = 'data:image/svg+xml;base64,' + encodedData

					var width = document.getElementById('kity_g_2').getBoundingClientRect().width;
					var height = document.getElementById('kity_g_2').getBoundingClientRect().height;
					newValue = ui.convertDataUri(newValue);
					var cells = graph.getSelectionCells();
					console.log(cells);
					...
					if (newValue != null && (newValue.length > 0 || cells.length > 0)) {
						...
						try {
							// 创建cells 如果是新增公式时
							if (cells.length == 0) {
								cells = [graph.insertVertex(graph.getDefaultParent(), null, '', 0, 0, width, height,
								...
								cells[0].latex = window.kfe.execCommand('get.source')
								select = cells;
			...
			window.openKformula = function () {
				window.editorUi.openKformula();
				return;
				window.editorUi.editor.graph.clearSelection();
				// new mxCell("Last updated 3 mins ago", new mxGeometry(0, 1, 260, 50), "html=1;shadow=0;dashed=0;shape=mxgraph.basic.corner_round_rect;dx=2;flipV=1;perimeter=none;whiteSpace=wrap;fillColor=#F7F7F7;strokeColor=#DFDFDF;fontColor=#6C767D;resizeWidth=1;fontSize=13;align=left;spacing=20;")
				$('.kf-model').css('visibility', 'visible')
				window.kfe.execCommand("render", "\\placeholder");
				window.kfe.execCommand("focus");
			...
			window.insertFreehand = function () { // 自由绘图
				if (graph.isEnabled()) {
			...
			window.selectSpecialSharacters = function () { // 特殊字符选中
				$('.special-sharacters-model').show();
			};

			window.savaData = function () { // 仅保存不退出
				window.editorUi.js2exe_SaveToAnswerArea(false, 1);		// yanwx, 1：手动保存, js2exe
			};

			// 重答，插入公式，特殊字符，保存
			var szHtmlNewFormula = '<a class="geButton hytButton" onclick="openKformula()">' + mxResources.get('insertFormula') + '</a>';		// 插入公式
			var szHtmlInsertFreehand = '<a class="geButton hytButton" onclick="insertFreehand()">' + mxResources.get('freehand') + '</a>';		// 自由绘图
			var szHtmlSpecialSharacters = '<a class="geButton hytButton" onclick="selectSpecialSharacters()">' + mxResources.get('SpecialSharacters') + '</a>';		// 特殊字符
			var szHtmlSaveData = '<a class="geButton hytButton" onclick="savaData()">' + mxResources.get('saveData') + '</a>';		// 保存
			$(editorUi.toolbar.container).append($(szHtmlNewFormula));
			$(editorUi.toolbar.container).append($(szHtmlInsertFreehand));
			$(editorUi.toolbar.container).append($(szHtmlSpecialSharacters));

			$.getJSON("resources/specialSharacters.json", function (data) {
				data.map((item, index) => {
					let itemHtml = `
					<p class="special-sharacters-title"  onclick="toggleText(${index})">
					...
					$('.special-sharacters-content').append($(itemHtml))
				})
				toggleText(0);
			});

			// 关闭便签本
			window.editorUi.closeLibrary(window.editorUi.scratchpad);
		}); // ready

		onok = function (editor) {
			console.log('diagramly-EditorUi.ok()')
	...
	EditorUi.prototype.init = function () {
		...
		if (!this.editor.chromeless || this.editor.editable) {
			...
			this.keyHandler.bindAction(70, true, 'insertFormula', true); // Ctrl+Shift+F
		...
		//初始化完成回显示
		if(window.parent.DrawioImageData)
			this.exe2js_LoadDataFromExe(window.parent.DrawioImageData);
	};
	...
	EditorUi.prototype.installSettings = function () {
		if (this.isSettingsEnabled()) {
			 ...
			 */
			// 修改默认尺寸大小
			if (urlParams['customMenu'] == '1') {
				mxSettings.setPageFormat(mxConstants.PAGE_FORMAT_HYTEXAM_PORTRAIT);
