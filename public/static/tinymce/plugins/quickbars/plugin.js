...
(function () {
    ...
    var setupButtons = function (editor) {
      ...
      editor.ui.registry.addMenuItem('quickimage', {
        icon: 'image',
        tooltip: 'Insert image',
        text:'Insert image',
        onAction: function () {
      ...
      editor.ui.registry.addButton('quicktable', {
        ...
        // 注册一个菜单项名称 menu/menubar
      editor.ui.registry.addMenuItem('quicktable', {
        icon: 'table',
        tooltip: 'Insert table',
        text:'Insert table',
        onAction: function () {
    ...
    var getTextSelectionToolbarItems = function (editor) {
      return getToolbarItemsOr(editor, 'quickbars_selection_toolbar', 'bold italic emphatic | quicklink h2 h3 blockquote');
    };
    ...
    var addToEditor = function (editor) {
      ...
      var hasImageOrgFile = function (node) {
        if(isImage(node)){
          if(node.nodeName==='FIGURE'){
            return node.childNodes[0].dataset && node.childNodes[0].dataset.upLoadUrl
          }else if(node.nodeName === 'IMG'){
            return node.dataset && node.dataset.upLoadUrl;
          }
        }
	  ...
	  var isFomulaImage = function (node) {
      if(node.nodeName==='FIGURE'){
        return node.childNodes[0].dataset && node.childNodes[0].dataset.formulaImage
      }else if(node.nodeName === 'IMG'){
        return node.dataset && node.dataset.formulaImage;
      }
      return  false;
	  };

    	  
	  var isMathTypeImage = function (node) {
      if(node.nodeName==='FIGURE'){
        return node.childNodes[0].className && node.childNodes[0].className == "mathtypeclient"
      }else if(node.nodeName === 'IMG'){
        return node.className && node.className == "mathtypeclient";
      }
      return  false;
	  };
	  
    var isFormatdebitlendbrushImage = function (node) {
      if(node.nodeName==='FIGURE'){
        return node.childNodes[0].dataset && node.childNodes[0].dataset.formatdebitlendImage
      }else if(node.nodeName === 'IMG'){
        return node.dataset && node.dataset.formatdebitlendImage;
      }
      return  false;
	  };

    //是否是画图工具
    var isDrawioImage = function (node) {
      if(node.nodeName==='FIGURE'){
        return node.childNodes[0].dataset && node.childNodes[0].dataset.drawioImage
      }else if(node.nodeName === 'IMG'){
        return node.dataset && node.dataset.drawioImage;
      }
      return  false;
	  };
    ...
    //图片源文件
    editor.ui.registry.addContextToolbar('imageorgfileselection', {
      predicate: hasImageOrgFile,
      items: 'imagetooldownOrgFile',
      position: 'node'
    });

	   //公式
    editor.ui.registry.addContextToolbar('formulaimageselection', {
      predicate: isFomulaImage,
      items: 'formula',
      position: 'node'
    });
   
     //公式
     editor.ui.registry.addContextToolbar('mathtypeimageselection', {
      predicate: isMathTypeImage,
      items: 'mathtype',
      position: 'node'
    });
    //借贷公式Formatdebitlendbrush
    editor.ui.registry.addContextToolbar('formatdebitlendbrushimageselection', {
      predicate: isFormatdebitlendbrushImage,
      items: 'formatdebitlendbrush',
      position: 'node'
    });

     //画图工具drawio
     editor.ui.registry.addContextToolbar('formatdrawioimageselection', {
      predicate: isDrawioImage,
      items: 'drawio',
      position: 'node'
    });
