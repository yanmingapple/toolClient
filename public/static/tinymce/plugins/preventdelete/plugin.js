	...
	function PreventDelete() {
		...
		this.keyWillDelete = function(evt) {
			...
			*/
			...
			return [8, 9, 13, 46,32,229].contains(c) || r(c, 48, 57) || r(c, 65, 90) || r(c, 96, 111) || r(c, 186, 192) || r(c, 219, 222)
		...
		tinymce.PluginManager.add('preventdelete', function(ed, link) {
			ed.on('keydown', function(evt) {
				...
				//禁用键盘输入编辑
				var conNoEdit = ed.getParam('conNoEdit',false)
				if(conNoEdit != undefined && conNoEdit == true){
				...
				for (var c = range.startContainer; !conNoEdit && c; c = c.nextSibling) {
					//此处改造：判断选择区域 、内部元素 是否包含不可编辑模块  1.局部选择 2 全选
					conNoEdit = conNoEdit || (self.check(c) ||  self.checkChildren(c))
				...
				//这里是也有一个bug,充前往后选择时候 ，应该与从后选择一直
				
				for (var c = range.endContainer; !conNoEdit && c; c = c.previousSibling) {
					//此处改造：判断选择区域 、内部元素 是否包含不可编辑模块  1.局部选择 2 全选
					conNoEdit = conNoEdit || (self.check(c) ||  self.checkChildren(c))
					if (range.startContainer === c)
						break
				}
				...
				if (back) {
					...
					var next = self.nextElement(range.startContainer)
			...
			ed.on('keyup', function (e) {
				...
				if(conNoEdit != undefined && conNoEdit == true){
					ed.undoManager.transact(function () {
					  return ed.setContent(ed.undoManager.data[ed.undoManager.getCurrentDataIndex()].content);
					});
			
					return self.cancelKey(e)
				}
