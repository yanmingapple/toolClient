tinymce.PluginManager.add('formatbrush', function(editor, url) {
    ...
    var fontStyle = {
        familyCN: "font-family:'宋体','Songti SC','SimSun'",
        familyEN: "font-family:'Times New Roman'",
        size: 'font-size:12pt;',
        retCN: function () {
          return this.familyCN + ';' + this.size;
        },
        retEN: function () {
          return this.familyEN + ';' + this.size;
        },
        retEC: function () {
          return this.familyEN + "'宋体','Songti SC','SimSun';" + this.size;
        },
        retCE: function () {
          return this.familyCN + "'Times New Roman';" + this.size;
        }
    };
    ...
    try {
      ...
      var __catchEl = (function (tagNames) { // dj 查找到table和工资元素，并替换成catch-el-的tag名，将原有dom放在catchEl中
        ...
        var __isFormulaNode = function (val) {
          var __sample = ['mathquill-rendered-math'];
          var __result = false;
        ...
        for (var i = 0; i < __arr.length; i++) {
          if (__arr[i].flag !== '') {
            ...
            for (var j = 0; j < __cols.length; j++) {
              ...
              __tmpPnl.innerHTML = '<tmp-td>' + __items.join(repeatStr('\t', 1)).replace(/\t/g, '</tmp-td><tmp-td>') + '</tmp-td>';
            ...
            if (i === 0 || __arr[i - 1].groupId !== __arr[i].groupId) {
              __result += '<table type="debit-lend" cellpadding="0" cellspacing="0">' + __arr[i].text.replace(/tmp-/g, '');
      ...
      var __edtFormat = function () {
        ...
        for (var i = 0; i < __result.length; i++) {
          ...
          try {
            if (item.getAttribute('class') && item.getAttribute('class').indexOf('borrow-lend-format-brush') > -1) {
              item.setAttribute('type', 'debit-lend');
            ...
            if (item.getAttribute('type') === 'debit-lend' && !isDebitLend) {
              continue;
            }
            var __findChar = '——';
            var __rows = 0;
            ...
            for (var j = 0; j < __trs.length; j++) {
              ...
              if (item.getAttribute('type') === 'debit-lend' && isDebitLend) {
                ...
                var __tmpPosition = __innerText.indexOf(__findChar);
    ...
    editor.ui.registry.addButton('formatbrush', {
      ...
      onAction: function () {
       var __tmpEl = document.createElement('tmp-el');
        ...
        var result =  __fcForAccounting(__tmpEl, false, true, false);
    ...
    editor.ui.registry.addMenuItem('formatbrush', {
      ...
      text:'formatbrush',
        onAction: function () {
          ...
          var result =  __fcForAccounting(__tmpEl, true, true, false);
          editor.setContent(result)
          editor.focus();
        }
    });
