...
(function () {
    ...
    var getImageSize = function (url,upLoadUrl) {
    ...
    var getCaptionContent = function(image){
      if(image?.parentNode?.nodeName == "FIGURE"){
        return image.parentNode.innerText;
      }
      return "";
    }
    ...
    //选择画图的特定样式
    var figuretype = ['formulaImage','drawioImage','formatdebitlendImage']
    var wrapInFigure = function (image) {
      
      var figuretypeClass= '';
      figuretype.forEach(type=>{
        if(image.dataset[type]) figuretypeClass =  type
      })

      var figureElm = DOM.create('figure', { class: 'image ' + figuretypeClass });
      DOM.insertAfter(figureElm, image);
      figureElm.appendChild(image);
      figureElm.appendChild(DOM.create('figcaption', { }, '题xxx图'));
   ...
   var getDataset = function (image,key) {
      return image?.dataset[key]
    };

    var setDatasetUrl = function (image, key,value) {
    ...
    var create = function (normalizeCss, data) {
      ...
      if (data.caption) {
        ...
        figure.appendChild(DOM.create('figcaption', {  }, data.captionContent??'题xxx图'));
    ...
    var read = function (normalizeCss, image) {
      return {
        ...
        upLoadUrl:getDataset(image,"upLoadUrl")??'',
        alt: getAlt(image),
        ...
        captionContent:hasCaption(image)?getCaptionContent(image):'',
        dpi:getDataset(image,"dpi")??'600',
    ...
    var write = function (normalizeCss, newData, image) {
      ...
      updateProp(image, oldData, newData, 'upLoadUrl', setDatasetUrl);
      updateProp(image, oldData, newData, 'dpi', setDatasetUrl);
    ...
    var insertImageAtCaret = function (editor, data) {
      ...
      //设置上传地址与dpi
      insertedElm.dataset["upLoadUrl"] = data.upLoadUrl;
      insertedElm.dataset["dpi"] = data.dpi;
       insertedElm.dataset["mceContenteditable"] = false;
       insertedElm.contentEditable = false;
    ...
    var writeImageDataToSelection = function (editor, data) {
      var image = getSelectedImage(editor);
      //设置上传地址与dpi
      image.dataset["upLoadUrl"] = data.upLoadUrl;
      image.dataset["dpi"] = data.dpi;
      // image.dataset["mceContenteditable"] = false;
    ...
    var makeTab = function (_info) {
      var items = [{
          ...
          browseTitle:'Browse for an image',
        }];
      return {
        title: 'Upload',
        name: 'upload',
    ...
    var fromImageData = function (image) {
      return {
        ...
        captionContent:image.captionContent,
        dpi:image.dpi,
        style: image.style,
        vspace: image.vspace,
        border: image.border,
    ...
    var toImageData = function (data, removeEmptyAlt) {
      return {
        ...
        captionContent:data.captionContent,
        dpi:data.dpi,
        hspace: data.hspace,
        vspace: data.vspace,
        border: data.border,
    ...
    var formFillFromMeta2 = function (info, data, meta) {
      ...
      if (info.hasImageCaption) {
        if (isBoolean(meta.caption)) {
          ...
          data.captionContent = meta.captionContent;
        }
      }
      if (info.hasAdvTab) {
    ...
    var calculateImageSize = function (helpers, info, state, api) {
      ...
      var upLoadUrl = data.upLoadUrl.value??'';
      var meta = data.src.meta || {};
      if (!meta.width && !meta.height && info.hasDimensions) {
        if (isNotEmpty(url)) {
          helpers.imageSize(url,upLoadUrl).then(function (size) {
            if (state.open) {
              if(api.getData().dpi){
                const dpi = Number.parseInt(api.getData().dpi??1);
                let width = Number.parseInt(size.width)/dpi*96
                let height = Number.parseInt(size.height)/dpi*96
                size.width = Number.parseFloat(width).toFixed(2); ;
                size.height =  Number.parseFloat(height).toFixed(2);;
    ...
    var changeFileInput = function (helpers, info, state, api) {
      ...
      head(data.fileinput).fold(function () {
        ...
        var updateSrcAndSwitchTab = function (url,upLoadUrl) {
          api.setData({
            src: {
              value: url,
              upLoadUrl:upLoadUrl,
        ...
        blobToDataUri(file).then(function (dataUrl) {
          ...
          if (info.automaticUploads) {
            helpers.uploadImage(blobInfo).then(function (result) {
              updateSrcAndSwitchTab(result.url,result.upLoadUrl);
              finalize();
            }).catch(function (err) {
              finalize();
    ...
    var imageSize = function (editor) {
      return function (url,upLoadUrl) {
        if (!isSafeImageUrl(editor, url)) {
          ...
          return getImageSize(editor.documentBaseURI.toAbsolute(url),upLoadUrl).then(function (dimensions) {
            return {
    ...
    var register = function (editor) {
      editor.ui.registry.addToggleButton('image', {
        ...
        onSetup: function (buttonApi) {
          ...
          return editor.selection.selectorChangedWithUnbind('img:not([data-mce-object],[data-mce-placeholder],[data-formatdebitlend-image],[data-formula-image]),figure.image', buttonApi.setActive).unbind;
        }
      });
